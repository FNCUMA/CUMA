...
<main class="container d-flex flex-column align-items-center justify-content-center">
    <section class="row d-flex flex-column justify-content-around my-3">

    <?php if (have_posts()):
        while (have_posts()): the_post();
            the_title('<h2 class="w-100 display-4 mb-3 text-center text-lg-left">','</h2>');
            ?>
    ...
    <?php
        ...
        <ul class="list-group col-lg-12 d-flex flex-row justify-content-between pr-0 mt-5 bg-white">
            <li class="list-group-item border-0"><?php previous_post_link('%link','%title','vidéo',TRUE); ?></li>
            <li class="list-group-item border-0"><?php next_post_link('%link','%title','vidéo',TRUE); ?></li>
        </ul>
